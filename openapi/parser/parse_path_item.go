...
func (p *parser) parsePathItem(
	...
	if len(item.AdditionalOperations) > 0 {
		...
		// Validate that additionalOperations entries do not contain any entry for the methods that can be defined by other fixed fields such as get, put, post, delete, options, head, patch, trace, or query.
