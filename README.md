...
- Generated sum types for oneOf
  ...
  - Field type discrimination: when variants share field names but differ in types (e.g., `{id: string}` vs `{id: integer}`), the decoder uses sequential variant matching to find the correct type at runtime
