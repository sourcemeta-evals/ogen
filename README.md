...
- Generated sum types for oneOf
  ...
  - Field type discrimination: when variants share field names but differ in types (e.g., `{id: string}` vs `{id: integer}`), the decoder peeks at the JSON type using `jx.Decoder.Next()` to dispatch to the correct variant at runtime
